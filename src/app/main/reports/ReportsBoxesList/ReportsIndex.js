...
function ReportsIndex(props) {
	...
	// const ReportsBoxesList = useSelector(selectBoards);
	...
	console.log('I am ReportsBoxesList',boards)
				...
				>
					{monthlyReportDownload.map(board => (
						<motion.div variants={item} className="w-224 h-224 p-16" key={board.id}>
							<Paper
								to={`/analysisreport/ReportsList/${board.url}`}
								className={clsx(
									classes.board,
								...
								role="button"
								component={Link}
								// onClick={event => handleDownload(ReportsTemplates.url)}
							>
...
export default withReducer('scrumboardApp', reducer)(ReportsIndex);
