...
import PermanentlyDisconnectedMeterTable from './PermanentlyDisconnectedMeterTable';
import { selectUsers } from '../../store/disconnectedmeterSlice';

function PermanentlyDisconnectedMeterHead(props) {
	...
	return (
		<motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1, transition: { delay: 0.2 } }}>
			<PermanentlyDisconnectedMeterTable
				columns={columns}
				data={filteredData}
				// onRowClick={(ev, row) => {
				// 	if (row) {
...
export default PermanentlyDisconnectedMeterHead;
