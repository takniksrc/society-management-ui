...
function BillingBlocksIndex(props) {
	...
	console.log('configSlice inside billing BillingBlocksIndex : ', configurationsData);
	console.log('billingBlockSlice inside billing BillingBlocksIndex : ', billingBlocksStatuses);
					...
					{billingBlocksStatuses?.map((board, index) => {
						...
						return board.billing_status === 'closed' || board.billing_status === 'init-in-progress' ? (
							<motion.div variants={item} className="p-16 w-auto" key={board.block_id}>
								<Paper
									to={`/billing/BillingBlocks/${board.name}/${board.block_id}`}
									className={clsx(
										classes.board,
							...
							<motion.div variants={item} className="p-16  w-auto" key={board.block_id}>
								<Paper
									to={`/billing/BillingBlocks/${board.name}/${board.block_id}/billing`}
									className={clsx(
										classes.board,
										'flex flex-col items-center justify-center w-full h-full rounded-16 py-24 shadow hover:shadow-lg'
									)}
...
export default withReducer('scrumboardApp', reducer)(BillingBlocksIndex);
